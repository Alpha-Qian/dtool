...
class DownloadFile:
    ...
    async def main(self, pre_task_num=1, auto=True, max_task_num=256):
        ...
        while self.process != self.file_size:
            ...
            print(
                f"process\t{self.process/self.file_size}\tleft\t{self.file_size - self.process}"
            )
            """   
            ...
            async for chunk in response.aiter_raw(16834):
                ...
                if self.process + len_chunk < self.end:
                    ...
                    async with mission.file_lock:
                        ...
                        if (i := mission.task_list.index(self) + 1) == len(
                            mission.task_list
                        ):
                            await mission.file.truncate()
                        elif self.process + len_chunk < mission.task_list[i].process:
                            i = await mission.file.read(
                                self.process + len_chunk - self.end
                            )  #
                            if i == check_chunk:
                                print("校验成功")
                                break
...
class WebResouseStream(DownloadBase, StreamBase):
    ...
    async def stream(self, block: Block):
        async for chunk in super().stream():
            ...
            if (
                ...
                and block.process + len_chunk >= self.iter_process + self._step
            ):
                self.allow_iter.set()
    ...
    async def __anext__(self):
        ...
        if (
            ...
            and self.iter_process + self._step > self.block_list[0].process
        ):
...
async def main():
    ...
    await asyncio.create_task(
        DownloadFile(url, path="./").main(pre_task_num=int(input("->")), auto=True)
    )
